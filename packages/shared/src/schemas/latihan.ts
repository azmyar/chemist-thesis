...
const validateMultipleChoiceOptions = (
	options: Array<z.infer<typeof latihanOptionInputSchema>> | undefined,
) => {
	if (!options) {
		return false;
	}
		...
		if (data.type === "multiple_choice") {
			if (!validateMultipleChoiceOptions(data.options)) {
				ctx.addIssue({
					...
					message:
						"Multiple choice membutuhkan minimal dua opsi dengan tepat satu jawaban benar",
					path: ["options"],
				});
			}
...
export const updateLatihanSchema = z
	...
	.superRefine((data, ctx) => {
		if (data.options !== undefined) {
			if (
				(data.type ?? "multiple_choice") === "multiple_choice" &&
